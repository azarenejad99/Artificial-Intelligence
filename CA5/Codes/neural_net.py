...
def make_neural_net_two_moons():
    ...
    Aweights = {}
    for i in range(40):
        _key = f'A{str(int(i/10))}{str(int(i % 10))}'
        Aweights[_key] = [
            Weight(f'w{_key}', random_weight()),
            Weight(f'w1{_key}', random_weight()),
            Weight(f'w2{_key}', random_weight())
        ]

    Aneurons = []
    for i in range(40):
        _key = f'A{str(int(i/10))}{str(int(i % 10))}'
        Aneurons.append(Neuron(_key, [i0, i1, i2], Aweights[_key]))

    wB = Weight('wB',  random_weight())
    Bweights = []
    for i in range(40):
        _key = f'A{str(int(i/10))}{str(int(i%10))}'
        Bweights.append(Weight(f'w{_key}B', random_weight()))

    B_weights = [wB]
    B_weights.extend(Bweights)
    B_inputs = [i0]
    B_inputs.extend(Aneurons)
    B = Neuron('B', B_inputs, B_weights)

    # P = PerformanceElem(B, 0.0)

    all_weights = []
    all_weights.extend(B_weights)
    for item in list(Aweights.values()):
        all_weights.extend(item)
    P = RegularizedPerformanceElem(B, 0.0)
    P.set_weights(all_weights)

    neus = []
    neus.extend(Aneurons)
    neus.append(B)
    net = Network(P, neus)
